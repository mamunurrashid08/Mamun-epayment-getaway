    ...
    appOpenButtons.forEach(button => {
        button.addEventListener('click', function(e) {
            ...
            if (appName === 'bkash' || appText.includes('bKash')) {
                ...
                // Rocket deep links
                appSchemes = ['rocket://'];
                intentUrls = ['intent://payment/#Intent;scheme=rocket;package=com.dbbl.mbs.apps.main;end'];
            ...
            // Try to open the app using the most reliable method for each platform
            if (isAndroid) {
                // For Android, try intent URL first (most reliable)
                if (intentUrls.length > 0) {
                    window.location.href = intentUrls[0];
                    return;
                }
            } else if (isIOS) {
                // For iOS, try universal links first if available
                if (universalLinks.length > 0) {
                    // Create a hidden anchor and trigger a click
                    const link = document.createElement('a');
                    link.setAttribute('href', universalLinks[0]);
                    link.style.display = 'none';
                    document.body.appendChild(link);
                    link.click();
                    document.body.removeChild(link);
                    
                    // Also try with direct location change as backup
                    setTimeout(() => {
            ...
            // Fallback to URI scheme for all platforms
            if (appSchemes.length > 0) {
                // Try with direct location change
                window.location.href = appSchemes[0];
                
                // For iOS, also try with iframe as backup
                if (isIOS) {
            ...
            // Use a longer timeout to give the app time to open
            setTimeout(() => {
                // Check if page is still visible (app didn't open)
                if (document.visibilityState !== 'hidden') {
                    // Direct to appropriate store based on platform
